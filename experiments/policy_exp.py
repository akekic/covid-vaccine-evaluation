...
if __name__ == "__main__":
    ...
    exp_dir = args.output_dir / (
        datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S.%f") + "_" + "policy_exp"
    )
    if args.save_output:
    ...
    for i, policy in enumerate(args.vaccination_policy):
        logger.info(
            f"Simulation of vaccination policy {policy} ({i+1}/{len(args.vaccination_policy)})"
        )
